...
#' @template n
#'
#' @return list of cumulative sample sizes. cumn[[2]][["T"]] contains cumulative sample size
#' of treatment group in second stage, etc.
#'
calc_cumn <- function(n) {
...
#' @template stagec
#'
#' @return list of cumulative allocation ratios. cumn[[2]][["T"]] = stagec[[1]][["T"]] + stagec[[2]][["T"]], etc.
#'
calc_cumc <- function(stagec) {
  cumc <- stagec
...
#' @template stagec
#'
#' @return list containing the stagewise sample sizes of the three groups.
#' n[[1]][["T"]] denotes first stage treamtment group sample size, etc.
#'
#' @examples
calc_n_from_c <- function(nT1, stagec) {
  n <- list()
  n[[2]] <- list()
  n[[1]][["T"]] <- nT1
...
#' @template n
#'
#' @return list containing the stagewise allocation ratios of the three groups, relative to the sample size of
#' the treatmnet group of the first stage. stagec[[1]][["P"]] = n[[1]][["P"]] / n[[1]][["T"]], etc.
#' 
calc_stagec <- function(n) {
  stagec <- list()
  stagec[[2]] <- list()
  stagec[[1]][["T"]] <- 1
...
#' @template cumc
#'
#' @return list containing the "gamma" factors.
#' gamma[[1]][["TP"]] = sqrt(var[["T"]] / cumc[["T"]] + var[["T"]] / cumc[["P"]]), etc.
#'
calc_gamma <- function(var, cumc) {
  gamma <- list()
  gamma[[1]] <- list()
  gamma[[2]] <- list()
  for (g in c("P", "C")) {
...
#' @template cumc
#' @template gamma
#' @return covariance matrix of the vector c(Z_TP1, Z_TP2, Z_TC1, Z_TC2).
#' @export
#'
calc_Sigma <- function(var, cumc, gamma) {
...
create_Design <- function(type_I_error = 0.025,
  ...
  A <- list()
  A[[2]] <- list()
  A[[1]][["TP"]] <- matrix(c(1, 0, 0, 0), nrow = 1, byrow = TRUE)
  A[[2]][["TP"]] <- matrix(c(0, 1, 0, 0), nrow = 1, byrow = TRUE)
  A[[1]][["TC"]] <- matrix(c(0, 0, 1, 0), nrow = 1, byrow = TRUE)
  A[[2]][["TC"]] <- matrix(c(0, 0, 0, 1), nrow = 1, byrow = TRUE)
...
calc_local_rejection_boundaries <- function(groups = "TP", D) {
  ...
  P[[paste0(groups, "1E")]] <- pmvnorm_(
    ...
    var = D$A_[[paste0(groups, "1")]] %*% D$Sigma %*% t(D$A_[[paste0(groups, "1")]]),
  ...
  sgn_low <- sign(pmvnorm_(
    ...
    var = D$A_[[paste0(groups, "12")]] %*% D$Sigma %*% t(D$A_[[paste0(groups, "12")]]),
...
opt_objective <- function(x, alpha, beta, distribution = c("norm", "t")){
  stagec <- list()
  stagec[[2]] <- list()
  stagec[[1]][["T"]] <- 1
  stagec[[1]][["P"]] <- x[1]
  ...
  objective_twostage(stagec = stagec,
                     b = b,
...
objective_twostage <-
  function(stagec,
    ...
    b[[2]][["TC"]][["efficacy"]] <- TC_local_lower$root
    ...
    if (return_everything) {
      ...
      D$power_TP <- pmvnorm_(
        ...
        var = D$A_[["TP1"]] %*% D$Sigma %*% t(D$A_[["TP1"]]),
        ...
        var = D$A_[["TP12_TC1"]] %*% D$Sigma %*% t(D$A_[["TP12_TC1"]]),
  ...
  function(x,
    ...
    if (return_everything) {
      ...
      D$power_TP <- pmvnorm_(
        ...
        var = D$A_[["TP1_TC1"]] %*% D$Sigma %*% t(D$A_[["TP1_TC1"]]),
        lower = c(D$b[[1]][["TP"]][["efficacy"]]),
        upper = c(pInf[[1]][["TP"]]),
        algorithm = Miwa(steps = D$maxpts)
        # algorithm = GenzBretz(maxpts = D$maxpts,
        ...
        var = D$A_[["TP12"]] %*% D$Sigma %*% t(D$A_[["TP12"]]),
