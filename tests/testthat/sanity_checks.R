...
# Compare results with work from Schloemer
schloemer_reference_optimal_singlestage <- function(D) {
  ...
  nmax <- opt$value
  return(list(cc = cc, nmax = nmax))
}

design1_list_kappa0 <- lapply(design1_list, function(x) if (x$kappa %==% 0) {
  return(x)
} else {
  return(NULL)
})
design1_list_kappa0[sapply(design1_list_kappa0, is.null)] <- NULL

test_that("Optimal single stage designs from Patrick Schloemer Dissertation agrees with our computation", {
  difcs <- sapply(
    design1_list_kappa0,
    function(D) max(abs(unlist(schloemer_reference_optimal_singlestage(D)$cc) - unlist(D$cc[[1]])))
  )
  difn <- sapply(
    design1_list_kappa0,
    function(D) max(abs(unlist(schloemer_reference_optimal_singlestage(D)$nmax) - unlist(D$maxn)))
  )
  expect_equal(difcs, rep(0, length(difcs)), tolerance = 1e-4)
  expect_equal(difcs, rep(0, length(difcs)), tolerance = 1e-4)
})
...
test_that("ASN from Patrick Schloemer Dissertation agrees with our computation", {
  expect_equal(
    sapply(design2_list, schloemer_reference_ASN),
    sapply(design2_list, function(x) x$ASN$H1)
  )
})
